...
export function useTeachingAssistants({ initialPage = 1, limit = 10, initialSearch = '' }: UseTeachingAssistantsOptions = {}) {
    ...
    try {
      ...
      if (res.success) {
        const arr = Array.isArray(res.data) 
          ? res.data 
          : (res.data?.data || res.data?.teachingAssistants || res.data?.students || res.data?.courses || res.data?.departments || []);
        setData(arr);
