...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center animate-in fade-in zoom-in duration-500 bg-brand-bg-card/30 rounded-[2rem] border-2 border-dashed border-brand-border">
      <div className="w-20 h-20 bg-brand-primary-50 text-brand-green-dark rounded-3xl flex items-center justify-center mb-6 shadow-inner">
        {icon}
      </div>
      ...
      {action && (
        <Button
          ...
          className="shadow-xl shadow-brand-green-dark/20 font-black uppercase tracking-widest text-xs px-8"
        >
          {action.label}
        </Button>
      )}
