...
const Modal = ({ isOpen, onClose, title, subtitle, children, size = 'md' }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
      ...
      <div className={`relative w-full ${sizes[size]} bg-brand-bg-card rounded-[2rem] shadow-overlay border border-brand-border overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-8 duration-500`}>
        <div className="card-header">
          <div className="flex items-start justify-between w-full gap-4">
            <div className="flex flex-col text-start flex-1 min-w-0">
              <h2 className="heading-1 m-0">
                {title}
              </h2>
              {subtitle && (
                <p className="text-caption mt-2">{subtitle}</p>
              )}
            </div>
            <button
              onClick={onClose}
