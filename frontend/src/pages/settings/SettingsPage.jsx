...
const SettingsPage = () => {
  ...
  const validatePassword = () => {
    const newErrors = {};
    if (!passwordData.currentPassword) newErrors.currentPassword = t('settings.currentPasswordRequired');
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-brand-text-primary tracking-tight m-0">{t('nav.settings')}</h1>
          <p className="text-brand-text-sub font-bold mt-1.5">{t('settings.subtitle')}</p>
        </div>
        <Badge variant="info" className="px-3 py-1.5 text-xs font-black uppercase tracking-widest">{user?.role?.replace('_', ' ')}</Badge>
      </div>
          ...
          <Card className="p-1.5">
            <nav className="space-y-0.5">
              {tabs.map((tab) => (
                ...
                >
                  <tab.icon size={16} className={`shrink-0 ${activeTab === tab.id ? 'text-white' : 'text-brand-text-muted'}`} />
        ...
        <div className="lg:col-span-4 xl:col-span-5 space-y-6">
          ...
          {activeTab === 'account' && (
            <Card>
              <div className="px-6 py-5 border-b border-brand-border">
                <h2 className="text-lg font-black text-brand-text-primary tracking-tight m-0">{t('settings.accountInfo')}</h2>
                <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.accountInfoDesc')}</p>
              </div>

              <div className="p-6 bg-gradient-to-r from-brand-primary-50/50 to-transparent border-b border-brand-border">
                <div className="flex items-center gap-5">
                  <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-brand-primary-400 to-brand-primary-600 flex items-center justify-center text-white text-xl font-black shadow-lg shrink-0">
                    {user?.firstName?.[0]}{user?.lastName?.[0]}
                  </div>
                  <div>
                    <h3 className="text-lg font-black text-brand-text-primary">{user?.firstName} {user?.lastName}</h3>
                    <p className="text-sm font-semibold text-brand-text-muted">{user?.email}</p>
                    <Badge variant="info" className="mt-1.5 text-[10px] px-2.5 py-0.5 font-black uppercase tracking-widest">{user?.role?.replace('_', ' ')}</Badge>
                  </div>
                </div>
              </div>
                ...
                <div className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="space-y-1.5">
                      ...
                      />
                      <p className="text-[10px] font-bold text-brand-text-muted uppercase tracking-widest">{t('settings.emailCannotBeChanged')}</p>
                    </div>
                  </div>
                </div>
                <div className="px-6 py-4 bg-surface-subtle/50 border-t border-brand-border flex justify-end gap-3">
                  <Button variant="outline" type="button" onClick={() => window.location.href='/profile'} className="text-xs font-black uppercase tracking-widest">{t('settings.viewProfile')}</Button>
                  <Button type="submit" disabled={loading} className="text-xs font-black uppercase tracking-widest">
                    {loading ? <Loader2 className="animate-spin" size={16} /> : t('common.saveChanges')}
                  </Button>
                </div>
              </form>
            </Card>
          ...
          {activeTab === 'security' && (
            <div className="space-y-6">
              <Card>
                <div className="px-6 py-5 border-b border-brand-border">
                  <h2 className="text-lg font-black text-brand-text-primary tracking-tight m-0">{t('settings.password')}</h2>
                  <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.passwordDesc')}</p>
                </div>
                <form onSubmit={handlePasswordChange}>
                  ...
                  <div className="px-6 py-4 bg-surface-subtle/50 border-t border-brand-border flex justify-end">
                    <Button type="submit" disabled={loading} className="text-xs font-black uppercase tracking-widest">
                      {loading ? <Loader2 className="animate-spin" size={16} /> : t('settings.updatePassword')}
                    </Button>
                  </div>
                </form>
              </Card>
              ...
              <Card>
                <div className="px-6 py-5 border-b border-brand-border">
                  <h2 className="text-lg font-black text-brand-text-primary tracking-tight m-0">{t('settings.twoFactor')}</h2>
                  <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.twoFactorDesc')}</p>
                </div>
                <div className="p-6 space-y-4">
                  {isSuperAdmin && !twoFactorEnabled && (
                    <div className="p-4 rounded-xl border-2 border-brand-accent-yellow bg-brand-accent-yellow/10 flex items-start gap-3">
                      ...
                      <p className="text-sm font-bold text-brand-text-primary">{t('settings.twoFactorWarning')}</p>
                    </div>
                  )}
                  <div className={`flex items-center justify-between p-4 rounded-xl border bg-brand-bg-page/30 hover:bg-brand-bg-page/50 transition-all ${
                    isSuperAdmin && !twoFactorEnabled ? 'border-brand-accent-yellow' : 'border-brand-border'
                  }`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-brand-text-primary">{t('settings.authenticatorApp')}</p>
                        <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.authenticatorDesc')}</p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl border border-brand-border bg-brand-bg-page/30 opacity-60">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-brand-text-primary">{t('settings.securityKeys')}</p>
                        <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.securityKeysDesc')}</p>
                      </div>
                    </div>
                    <Badge variant="neutral" className="text-[10px] font-black uppercase tracking-widest">{t('common.comingSoon')}</Badge>
                  </div>
                </div>
              </Card>
            </div>
          ...
          {activeTab === 'appearance' && (
            <Card>
              <div className="px-6 py-5 border-b border-brand-border">
                <h2 className="text-lg font-black text-brand-text-primary tracking-tight m-0">{t('settings.appearanceTitle')}</h2>
                <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.appearanceDesc')}</p>
              </div>
              <div className="p-6 space-y-6">
                <div className="flex items-center justify-between p-4 rounded-xl border border-brand-border bg-brand-bg-page/30">
                  <div className="flex items-center gap-4">
                    <div className="h-10 w-10 rounded-xl bg-brand-bg-card border border-brand-border flex items-center justify-center shadow-sm">
                      {isDark ? <Moon size={18} className="text-brand-navy" /> : <Sun size={18} className="text-brand-accent-yellow" />}
                    </div>
                    <div>
                      <p className="text-sm font-bold text-brand-text-primary">{t('settings.darkMode')}</p>
                      <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.darkModeDesc')}</p>
                    </div>
                  </div>
                  <button 
                    onClick={toggleTheme}
                    className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-brand-primary-500/30 shrink-0 ${isDark ? 'bg-brand-primary-500' : 'bg-slate-200'}`}
                  >
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border border-brand-border bg-brand-bg-page/30">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-bold text-brand-text-primary">{t('settings.language')}</p>
                      <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.languageDesc')}</p>
                    </div>
                  </div>
                  <div className="flex gap-1.5">
                    <button 
                      ...
                      className={`px-3.5 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${language === 'en' ? 'bg-brand-navy text-white shadow-sm' : 'bg-surface-subtle text-brand-text-secondary hover:bg-brand-navy/10'}`}
                    >
                      EN
                    </button>
                    <button 
                      ...
                      className={`px-3.5 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${language === 'ar' ? 'bg-brand-navy text-white shadow-sm' : 'bg-surface-subtle text-brand-text-secondary hover:bg-brand-navy/10'}`}
                    >
                      AR
                    </button>
                  </div>
          ...
          {activeTab === 'system' && isAdmin && (
            <Card>
              <div className="px-6 py-5 border-b border-brand-border">
                <h2 className="text-lg font-black text-brand-text-primary tracking-tight m-0">{t('settings.systemAdmin')}</h2>
                <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.systemAdminDesc')}</p>
              </div>
              <div className="p-6 space-y-4">
                <div className="flex items-center justify-between p-4 rounded-xl border border-brand-border bg-brand-bg-page/30 hover:bg-brand-bg-page/50 transition-all">
                  <div className="flex items-center gap-4">
                    <div className="h-10 w-10 rounded-xl bg-brand-bg-card border border-brand-border flex items-center justify-center shadow-sm">
                      <Database size={18} className="text-brand-navy" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-brand-text-primary">{t('settings.maintenanceMode')}</p>
                      <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.maintenanceDesc')}</p>
                    </div>
                  </div>
                  <Badge variant="info" className="text-[10px] font-black uppercase tracking-widest">{t('common.disabled')}</Badge>
                </div>
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border border-brand-border bg-brand-bg-page/30 hover:bg-brand-bg-page/50 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-bold text-brand-text-primary">{t('settings.registrationLock')}</p>
                      <p className="text-xs font-semibold text-brand-text-muted mt-0.5">{t('settings.registrationDesc')}</p>
                    </div>
                  </div>
                  <Badge variant="success" className="text-[10px] font-black uppercase tracking-widest">{t('common.open')}</Badge>
                </div>
              </div>
            </Card>
          )}
