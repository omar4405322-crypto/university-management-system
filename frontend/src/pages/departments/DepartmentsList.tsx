    ...
    <div className="section-gap animate-page">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white dark:bg-slate-900 p-4 rounded-2xl border border-brand-border dark:border-brand-border shadow-soft">
          ...
          <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-brand-text-muted group-focus-within:text-brand-green-dark transition-colors" />
          <Input
        ...
        {!isCollegeAdmin && (
          <div className="relative md:max-w-xs w-full group">
            <Building className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-brand-text-muted group-focus-within:text-brand-green-dark transition-colors" />
            <Select
          ...
          {filteredDepartments.map((dept) => {
            ...
            <Card
              ...
              className={`group border-none shadow-soft hover:-translate-y-2 duration-500 overflow-hidden flex flex-col ${
                isSelected ? 'ring-2 ring-brand-green-dark bg-brand-green-dark/5' : ''
            ...
            >
              <div className={`flex-grow relative ${isCompact ? 'p-5' : 'p-8'}`}>
                ...
                <div className={`flex justify-between items-start ${isCompact ? 'mb-4' : 'mb-6'}`}>
                  <div className={`${isCompact ? 'w-10 h-10 ml-6' : 'w-14 h-14 ml-8'} rtl:mr-8 rtl:ml-0 bg-brand-primary-50 dark:bg-brand-primary-900/10 rounded-2xl flex items-center justify-center text-brand-green-dark group-hover:scale-110 group-hover:bg-brand-green-dark group-hover:text-white transition-all duration-500 shadow-inner`}>
                ...
                <div className="space-y-1">
                  <p className="label-stat text-brand-green-dark">{dept.college?.name}</p>
                  <h3 className={`${isCompact ? 'text-xl' : 'text-2xl'} font-black text-brand-text-primary dark:text-brand-text-main tracking-tight group-hover:text-brand-green-dark transition-colors duration-300`}>
                ...
                <div className={`grid grid-cols-2 gap-4 ${isCompact ? 'mt-4 pt-4' : 'mt-8 pt-8'} border-t border-brand-border dark:border-brand-border`}>
                  ...
                  <div className={`${isCompact ? 'p-2' : 'p-4'} bg-surface-subtle dark:bg-slate-800/50 rounded-2xl text-center group-hover:bg-brand-green-dark group-hover:text-white transition-colors duration-500`}>
              ...
              <div className="px-8 py-5 bg-surface-subtle dark:bg-slate-800/30 border-t border-brand-border dark:border-brand-border mt-auto flex justify-between items-center">
                <button
                  className="text-brand-green-dark font-black text-[11px] uppercase tracking-widest hover:text-brand-primary-600 flex items-center gap-2 transition-colors"
                ...
                <button
                  className="text-brand-navy-500 dark:text-brand-text-main font-black text-[11px] uppercase tracking-widest hover:text-brand-green-dark flex items-center gap-2 transition-colors"
      ...
      >
        {activeDrawerId && (
          <Suspense fallback={<div className="flex justify-center p-12"><Loader2 className="animate-spin text-brand-green-dark" size={32} /></div>}>
