  ...
  if (!isAdminOrDoctor) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in duration-700">
        ...
        <h2 className="text-2xl font-black text-brand-text-main tracking-tight uppercase italic">{t('attendance.accessDenied')}</h2>
        <p className="text-brand-text-sub font-bold mt-2 max-w-sm">{t('attendance.accessDeniedDesc')}</p>
      </div>
    );
  }

  const statusButtons = [
    { id: 'PRESENT', label: t('attendance.present'), icon: CheckCircle2, color: 'text-brand-green', bg: 'bg-brand-green/10', active: 'bg-brand-green text-white shadow-lg shadow-brand-green/20' },
    { id: 'LATE', label: t('attendance.late'), icon: Clock, color: 'text-brand-yellow', bg: 'bg-brand-yellow/10', active: 'bg-brand-yellow text-white shadow-lg shadow-brand-yellow/20' },
    { id: 'ABSENT', label: t('attendance.absent'), icon: XCircle, color: 'text-rose-500', bg: 'bg-rose-500/10', active: 'bg-rose-500 text-white shadow-lg shadow-rose-500/20' },
  ];
                ...
                <table className="w-full text-right" dir="rtl">
                  <thead>
                    <tr className="bg-surface-subtle dark:bg-slate-800/50 border-b border-brand-border">
                      ...
                      <th className="px-6 py-4 text-center label-stat">{t('attendance.statusColumn')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-brand-border">
                    {students.map((student) => (
                      <tr key={student.id} className="hover:bg-surface-subtle dark:hover:bg-slate-800/30 transition-colors">
                        <td className="px-6 py-5">
                          <span className="font-black text-brand-navy tracking-tight">{student.studentId}</span>
                        </td>
                        <td className="px-6 py-5">
