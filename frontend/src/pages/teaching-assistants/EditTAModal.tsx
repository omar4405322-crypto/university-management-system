...
interface EditTeachingAssistantModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  teachingAssistant: any;
}

const EditTeachingAssistantModal: React.FC<EditTeachingAssistantModalProps> = ({ isOpen, onClose, onSuccess, teachingAssistant }) => {
  ...
  useEffect(() => {
    if (teachingAssistant) {
      setFormData({
        firstName: (teachingAssistant.user?.email || '').split('@')[0] || '',
        lastName: '',
        phone: teachingAssistant.phone || '',
        specialty: teachingAssistant.specialty || '',
        departmentId: teachingAssistant.departmentId ? teachingAssistant.departmentId.toString() : '',
        isActive: teachingAssistant.isActive !== false,
      });

      // Filter and pre-fill assigned courses
      if (courses.length > 0) {
        const assignedIds = courses
          .filter((course) => course.specialization === teachingAssistant.id)
        ...
        if (coursesRes.success) {
          ...
          if (teachingAssistant) {
            const assignedIds = allCourses
              .filter((course: any) => course.specialization === teachingAssistant.id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.firstName || !formData.lastName) {
      showToast(t('teachingAssistants.fillRequired', 'يرجى ملء الحقول المطلوبة'), 'error');
      return;
    }
    ...
    try {
      ...
      // 1. Update teachingAssistant personal details & department
      ...
      const teachingAssistantResult = await teachingAssistantsService.updateTeachingAssistant(teachingAssistant.id.toString(), teachingAssistantData);

      if (!teachingAssistantResult.success) {
        showToast(teachingAssistantResult.message || t('teachingAssistants.updateError', 'حدث خطأ أثناء التحديث'), 'error');
        setLoading(false);
        return;
      }
      ...
      const addPromises = toAdd.map((courseId) =>
        coursesService.updateCourse(courseId.toString(), { specialization: teachingAssistant.id })
      );

      const removePromises = toRemove.map((courseId) =>
        coursesService.updateCourse(courseId.toString(), { specialization: null })
      );
      ...
      showToast(
        error.response?.data?.message || t('teachingAssistants.updateError', 'حدث خطأ أثناء التحديث'),
        'error'
      );
    } finally {
      setLoading(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-brand-bg-card dark:bg-brand-bg-elevated rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto border border-brand-border dark:border-brand-border flex flex-col text-start">
        ...
        <div className="px-6 py-4 border-b border-brand-border dark:border-brand-border flex justify-between items-center bg-brand-bg-page/50 dark:bg-brand-bg-elevated/50">
          <div>
            <h2 className="text-xl font-bold text-brand-text-primary dark:text-brand-text-main">
              {t('teachingAssistants.editTitle', 'تعديل بيانات الأستاذ')}: {teachingAssistant?.specialization}
            </h2>
            <p className="text-sm text-brand-text-secondary dark:text-brand-text-muted mt-0.5">
              {t('teachingAssistants.editDesc', 'تعديل البيانات الشخصية والمقررات المسندة للأستاذ')}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex border-b border-brand-border dark:border-brand-border bg-brand-bg-page/20 px-6">
          ...
          >
            <User size={16} />
            {t('teachingAssistants.personalTab', 'البيانات الشخصية')}
          </button>
          <button
            type="button"
          ...
          >
            <Building2 size={16} />
            {t('teachingAssistants.departmentTab', 'القسم الرئيسي')}
          </button>
          <button
            type="button"
          ...
          >
            <BookOpen size={16} />
            {t('teachingAssistants.coursesTab', 'المقررات المسندة')}
          </button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-brand-text-primary dark:text-brand-text-secondary flex items-center gap-2 ml-1">
                  ...
                  {t('teachingAssistants.firstName', 'الاسم الأول')} <span className="text-error">*</span>
                </label>
                <Input
                  name="firstName"
                  value={formData.firstName}
                  onChange={handleChange}
                  placeholder={t('teachingAssistants.firstNamePlaceholder', 'أدخل الاسم الأول')}
                  required
                />
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-brand-text-primary dark:text-brand-text-secondary flex items-center gap-2 ml-1">
                  <User size={14} className="text-brand-text-muted dark:text-brand-text-secondary" />{' '}
                  {t('teachingAssistants.lastName', 'الاسم الأخير')} <span className="text-error">*</span>
                </label>
                <Input
                  name="lastName"
                  value={formData.lastName}
                  onChange={handleChange}
                  placeholder={t('teachingAssistants.lastNamePlaceholder', 'أدخل الاسم الأخير')}
                  required
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={handleChange}
                  placeholder={t('teachingAssistants.phonePlaceholder', 'أدخل رقم الهاتف')}
                />
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-brand-text-primary dark:text-brand-text-secondary flex items-center gap-2 ml-1">
                  ...
                  {t('teachingAssistants.specialty', 'التخصص')}
                </label>
                <Input
                  name="specialty"
                  value={formData.specialty}
                  onChange={handleChange}
                  placeholder={t('teachingAssistants.specialtyPlaceholder', 'أدخل التخصص')}
                />
              </div>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-brand-text-primary dark:text-brand-text-secondary flex items-center gap-2 ml-1">
                  ...
                  {t('teachingAssistants.primaryDept', 'القسم الرئيسي')}
                </label>
                <select
                  name="departmentId"
                ...
                >
                  <option value="">{t('teachingAssistants.selectDept', 'اختر القسم الرئيسي')}</option>
            ...
            <div className="space-y-4">
              <div className="border-b border-brand-border dark:border-brand-border pb-2">
                <h3 className="text-sm font-bold text-brand-text-primary dark:text-brand-text-main">
                  {t('teachingAssistants.coursesList', 'قائمة المقررات الدراسية')}
                </h3>
                <p className="text-xs text-brand-text-secondary dark:text-brand-text-muted mt-1">
                  {t('teachingAssistants.coursesInstructions', 'حدد المقررات الدراسية التي يقوم الأستاذ بتدريسها عبر جميع الكليات والأقسام')}
                </p>
              </div>

              {courses.length === 0 ? (
                <div className="text-center py-8 text-brand-text-secondary">
                  {t('teachingAssistants.noCourses', 'لا توجد مقررات دراسية متاحة')}
                </div>
              ) : (
...
export default EditTeachingAssistantModal;
