...
const CourseDetails = () => {
  ...
  const breadcrumbItems = [
    ...
    ...(course.department?.college?.name
      ? [{ label: course.department.college.name, link: `/colleges/${course.department.college.id}` }]
      : []),
    ...
    <div className="section-gap animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <Card className="flex items-center gap-4">
          <div className="p-3 rounded-2xl bg-brand-primary-50 text-brand-primary-500">
            <Users size={24} />
          </div>
          <div>
            <p className="label-stat">{t('courses.students')}</p>
            <p className="text-2xl font-black">{course._count?.students ?? course.students?.length ?? 0}</p>
          </div>
        </Card>
        <Card className="flex items-center gap-4">
