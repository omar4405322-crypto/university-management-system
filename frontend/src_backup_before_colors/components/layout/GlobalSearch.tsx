...
const GlobalSearch = () => {
  ...
  const modal = open ? createPortal(
    <div className="fixed inset-0 z-[200] bg-brand-navy/60 backdrop-blur-sm flex items-start justify-center pt-[10vh] px-4">
                    ...
                    {section.items.map((item: { id: string, path: string, title: string, meta?: string }, localIdx: number) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-start px-3 py-3 rounded-2xl flex items-center justify-between transition-colors ${isSelected ? 'bg-brand-primary-500/10 text-brand-primary-600' : 'hover:bg-surface-subtle text-brand-text-primary dark:text-brand-text-main'}`}
                        >
                          ...
                          {isSelected && <span className="text-xs font-bold text-brand-primary-500 opacity-60 px-2 py-1 bg-brand-primary-500/10 rounded-lg">Enter ↵</span>}
                        </button>
                      );
                    })}
                  </div>
  ...
  return (
    <>
      <button
        ...
        className="hidden md:flex items-center justify-between w-64 lg:w-80 h-10 px-4 rounded-xl border border-brand-border bg-surface-subtle hover:bg-brand-bg-card hover:border-brand-primary-500/50 transition-all group"
      >
        <div className="flex items-center gap-2 text-brand-text-muted group-hover:text-brand-primary-500 transition-colors">
          <Search size={16} />
