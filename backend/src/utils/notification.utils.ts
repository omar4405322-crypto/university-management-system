...
export const notifyRole = async ({ role, title, message, type = 'info' }: NotifyRoleParams) => {
  try {
    ...
    return await Promise.all(users.map((user: { id: number }) => 
 ...
 */
export const notifyStudentsInCourse = async({ courseId, title, message, type = 'info' }: NotifyStudentsParams) => {
  try {
    ...
    const notifications = course.enrollments.map(({ student }: { student: { userId: number } }) => ({
 ...
 */
export const notifyAdminsOfNewRequest = async ({ role, firstName, lastName, departmentId }: NotifyAdminsParams) => {
  try {
    const parsedDeptId = typeof departmentId === 'string' ? parseInt(departmentId, 10) : departmentId;
    ...
    admins.forEach((admin: { id: number; role: string; collegeId: number | null; departmentId: number | null }) => {
