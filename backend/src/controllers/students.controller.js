...
exports.deleteStudent = catchAsync(async (req, res, next) => {
  ...
  await prisma.$transaction(async (tx) => { 
    ...
    await tx.payment.deleteMany({ where: { studentId: parseInt(id) } }); 
    await tx.quizSubmission.deleteMany({ where: { studentId: parseInt(id) } }); 
    await tx.taskSubmission.deleteMany({ where: { studentId: parseInt(id) } }); 
    
    const successMetric = await tx.studentSuccessMetric.findUnique({ 
      where: { studentId: parseInt(id) } 
    });
    if (successMetric) { 
      await tx.studentSuccessMetric.delete({ where: { studentId: parseInt(id) } }); 
    } 
