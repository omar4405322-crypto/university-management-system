...
exports.getQuizResults = catchAsync(async (req, res, next) => {
  ...
  if (courseScope3 && Object.keys(courseScope3).length) {
    if (courseScope3.department && quiz.submissions.some(s => s.quiz?.course?.department?.collegeId !== courseScope3.department.collegeId)) return next(new AuthorizationError('Access denied'));
    // For departmentId check, we need to ensure quiz.course.departmentId matches
    if (courseScope3.departmentId && quiz.submissions.some(s => s.quiz?.course?.departmentId !== courseScope3.departmentId)) return next(new AuthorizationError('Access denied'));
  }
