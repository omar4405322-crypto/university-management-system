...
export const getTimetables = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { user } = req;
  const { collegeId, departmentId, academicYear, semester, status } = req.query as Record<string, string>;
 ...
 * @access  Private
 */
export const getTimetableById = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (deptScope && Object.keys(deptScope).length) {
    if (deptScope.collegeId && timetable.collegeId !== deptScope.collegeId) return next(new AuthorizationError('Access denied'));
    if (deptScope.id && timetable.departmentId !== deptScope.id) return next(new AuthorizationError('Access denied'));
  }
 ...
 */
export const createTimetable = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!collegeId || !departmentId || !academicYear || !semester || !title) {
    return next(new AppError('Faculty, Department, Academic Year, Semester, and Title are required', 400));
  }

  // Enforce scope for creation
  const deptScope: any = getScopeWhere(req.user!, 'department');
  if (deptScope && Object.keys(deptScope).length) {
    if (deptScope.collegeId && parseInt(collegeId as string) !== deptScope.collegeId) return next(new AuthorizationError('Access denied'));
    if (deptScope.id && parseInt(departmentId as string) !== deptScope.id) return next(new AuthorizationError('Access denied'));
  }
  ...
  if (existing) {
    return next(new AppError('A timetable for this Faculty, Department, Year, and Semester combination already exists.', 400));
  }
 ...
 * @access  Private (Admin)
 */
export const updateTimetable = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (deptScope && Object.keys(deptScope).length) {
    if (deptScope.collegeId && existing.collegeId !== deptScope.collegeId) return next(new AuthorizationError('Access denied'));
    if (deptScope.id && existing.departmentId !== deptScope.id) return next(new AuthorizationError('Access denied'));
  }
 ...
 * @access  Private (Admin)
 */
export const deleteTimetable = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const existing = await prisma.timetable.findUnique({ where: { id: parseInt(req.params.id as string) } });
  if (!existing) return next(new NotFoundError('Timetable not found'));
  const deptScope: any = getScopeWhere(req.user!, 'department');
  if (deptScope && Object.keys(deptScope).length) {
    if (deptScope.collegeId && existing.collegeId !== deptScope.collegeId) return next(new AuthorizationError('Access denied'));
...
export const publishTimetable = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const timetable = await prisma.timetable.update({
    where: { id: parseInt(req.params.id as string) },
...
export const unpublishTimetable = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const timetable = await prisma.timetable.update({
    where: { id: parseInt(req.params.id as string) },
    data: { status: 'DRAFT' }
  });
