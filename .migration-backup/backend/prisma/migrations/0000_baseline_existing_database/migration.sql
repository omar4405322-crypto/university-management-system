...
-- CreateEnum
CREATE TYPE "Role" AS ENUM ('ADMIN', 'COLLEGE_ADMIN', 'DEPARTMENT_ADMIN', 'DOCTOR', 'STUDENT', 'SUPER_ADMIN', 'TEACHING_ASSISTANT');

-- CreateEnum
CREATE TYPE "AdminRole" AS ENUM ('ACADEMIC_DEAN', 'FINANCE', 'IT_SUPPORT', 'REGISTRAR');

-- CreateEnum
CREATE TYPE "RequestStatus" AS ENUM ('APPROVED', 'PENDING', 'REJECTED');

-- CreateEnum
CREATE TYPE "RiskLevel" AS ENUM ('CRITICAL', 'HIGH', 'LOW', 'MEDIUM');

-- CreateEnum
CREATE TYPE "AttendanceStatus" AS ENUM ('ABSENT', 'EXCUSED', 'LATE', 'PRESENT');

-- CreateEnum
CREATE TYPE "AttendanceMethod" AS ENUM ('MANUAL', 'QR', 'RFID');

-- CreateEnum
CREATE TYPE "PaymentType" AS ENUM ('LIBRARY', 'OTHER', 'REGISTRATION', 'TUITION');

-- CreateEnum
CREATE TYPE "PaymentStatus" AS ENUM ('CANCELLED', 'OVERDUE', 'PAID', 'PENDING');

-- CreateEnum
CREATE TYPE "ExamType" AS ENUM ('FINAL', 'MIDTERM', 'QUIZ');

-- CreateEnum
CREATE TYPE "ExamQuestionType" AS ENUM ('MCQ', 'SHORT_ANSWER', 'TRUE_FALSE');

-- CreateEnum
CREATE TYPE "ExamSubmissionStatus" AS ENUM ('GRADED', 'PENDING');

-- CreateEnum
CREATE TYPE "ExamViolationType" AS ENUM ('BLUR', 'COPY_PASTE', 'FULLSCREEN_EXIT', 'RIGHT_CLICK', 'TAB_SWITCH');

-- CreateEnum
CREATE TYPE "MaterialType" AS ENUM ('LECTURE', 'TUTORIAL');

-- CreateEnum
CREATE TYPE "EnrollmentStatus" AS ENUM ('BLOCKED', 'COMPLETED', 'ENROLLED', 'FAILED', 'WITHDRAWN');

-- CreateEnum
CREATE TYPE "ScheduleRequestType" AS ENUM ('DELETE_SLOT', 'NEW_SLOT', 'OVERRIDE', 'UPDATE_SLOT');

-- CreateEnum
CREATE TYPE "SlotType" AS ENUM ('LAB', 'LECTURE', 'SECTION');

-- CreateEnum
CREATE TYPE "TAStatus" AS ENUM ('ACTIVE', 'INACTIVE', 'ON_LEAVE');
...
CREATE TABLE "AbsenceThresholdPolicy" (
    ...
    "departmentId" INTEGER,
    "courseId" INTEGER,
    "maxAbsencePercent" DOUBLE PRECISION NOT NULL DEFAULT 25,
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" TIMESTAMP(3) NOT NULL,
...
-- CreateIndex
CREATE INDEX "AttendanceSession_isActive_expiresAt_idx" ON "AttendanceSession"("isActive", "expiresAt");
...
-- CreateIndex
CREATE UNIQUE INDEX "AbsenceThresholdPolicy_departmentId_courseId_key" ON "AbsenceThresholdPolicy"("departmentId", "courseId");
...
-- CreateIndex
CREATE INDEX "ScheduleOverride_doctorId_idx" ON "ScheduleOverride"("doctorId");
...
-- AddForeignKey
ALTER TABLE "RfidDevice" ADD CONSTRAINT "RfidDevice_roomId_fkey" FOREIGN KEY ("roomId") REFERENCES "Room"("name") ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE "ScheduleChangeRequest" ADD CONSTRAINT "ScheduleChangeRequest_departmentId_fkey" FOREIGN KEY ("departmentId") REFERENCES "Department"("id") ON DELETE SET NULL ON UPDATE CASCADE;
