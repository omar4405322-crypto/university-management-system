  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="form-section">
        {error && (
          <div className="p-4 rounded-xl bg-rose-50 dark:bg-rose-900/20 border border-rose-100 text-rose-600 text-sm font-bold">
            {error}
          </div>
        )}
        ...
        <div className="space-y-1.5">
          <Select
            ...
            {...register('courseId')}
           label={<>{t('exams.selectCourse')} *</>} error={errors.courseId?.message}>
        ...
        <div className="space-y-1.5">
          <label htmlFor="input-c14fc7" className="text-sm font-bold text-brand-text-main ml-1">
            Exam Date *
          </label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label htmlFor="input-5aa898" className="text-sm font-bold text-brand-text-main ml-1">
              Start Time *
            </label>
            <Input type="time" {...register('startTime')} />
            {errors.startTime && (
              <p className="text-rose-500 text-xs mt-1">{errors.startTime.message}</p>
            )}
          </div>
          <div className="space-y-1.5">
            <label htmlFor="input-3ac857" className="text-sm font-bold text-brand-text-main ml-1">
              End Time *
            </label>
            <Input type="time" {...register('endTime')} />
            {errors.endTime && (
              <p className="text-rose-500 text-xs mt-1">{errors.endTime.message}</p>
            )}
          </div>
        </div>

        <div className="space-y-1.5">
          <label htmlFor="input-5a567e" className="text-sm font-bold text-brand-text-main ml-1">
            Location / Room
          </label>
          <Input placeholder="e.g. Auditorium A, Hall 302" {...register('room')} />
          ...
          <Card noPadding className="border-none shadow-soft overflow-hidden">
            ...
            <div className="p-6 border-t border-brand-border dark:border-brand-border">
              <label className="flex items-center justify-between cursor-pointer group">
                <span className="label-stat group-hover:text-brand-text-primary transition-colors">
                  Upcoming Only
                </span>
                <div className="relative">
                  ...
                  />
                  <div
                    className={`w-10 h-5 rounded-full transition-colors duration-300 ${upcomingOnly ? 'bg-brand-primary-500' : 'bg-slate-200 dark:bg-slate-700'}`}
                  />
                  <div
                    className={`absolute top-1 left-1 w-3 h-3 rounded-full bg-white transition-transform duration-300 ${upcomingOnly ? 'translate-x-5' : ''}`}
                  />
                </div>
              </label>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {exams.map((exam) => (
                <Card
                  key={exam.id}
                  noPadding
                  className="group hover:-translate-y-1 duration-300 border-none shadow-soft overflow-hidden"
                >
                  <div className="p-6">
