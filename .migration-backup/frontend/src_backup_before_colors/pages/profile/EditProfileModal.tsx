    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md transition-all duration-300">
      <div className="bg-brand-bg-card dark:bg-brand-bg-elevated rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in duration-300 border border-brand-border dark:border-brand-border">
        ...
        <div className="flex justify-between items-center p-6 sm:p-8 border-b border-brand-border dark:border-brand-border bg-brand-bg-page/50 dark:bg-brand-bg-elevated/50">
          <div>
            <h2 className="text-2xl font-black text-brand-text-primary dark:text-brand-text-main tracking-tight">
              Profile Settings
            </h2>
            <p className="text-sm text-brand-text-secondary dark:text-brand-text-muted mt-1">
        ...
        <div className="p-6 sm:p-8 max-h-[70vh] overflow-y-auto no-scrollbar">
          {error && (
            <div className="mb-6 p-4 bg-error/10 dark:bg-error/20 text-error dark:text-error text-sm font-bold rounded-2xl border border-error/20 dark:border-error/30 flex items-center gap-3">
              <Info size={18} />
              {error}
            </div>
          )}
          {success && (
            <div className="mb-6 p-4 bg-success/10 dark:bg-success/20 text-success dark:text-success text-sm font-bold rounded-2xl border border-success/20 dark:border-success/30 flex items-center gap-3">
              <Info size={18} />
              {success}
            </div>
          )}
            ...
            <form onSubmit={generalForm.handleSubmit(onGeneralSubmit)} className="form-section">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label
                    htmlFor="input-5b1f78"
                    className="text-xs font-black text-brand-text-muted uppercase tracking-widest ml-1"
                  >
                    First Name
                  </label>
                  <div className="relative group">
                    <User
                      className="absolute left-4 top-1/2 -translate-y-1/2 text-brand-text-muted group-focus-within:text-info transition-colors"
                      size={18}
                    />
                    <input
                      type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  {generalForm.formState.errors.phone && (
                    <p className="text-rose-500 text-xs mt-1">
                      {generalForm.formState.errors.phone.message}
                    </p>
                  )}
                </div>
                <div className="space-y-2">
                  <label
                    htmlFor="input-dbf5fa"
                    className="text-xs font-black text-brand-text-muted uppercase tracking-widest ml-1"
                  >
                    Gender
                  </label>
                  ...
                  {generalForm.formState.errors.gender && (
                    <p className="text-rose-500 text-xs mt-1">
                      {generalForm.formState.errors.gender.message}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                {generalForm.formState.errors.birthDate && (
                  <p className="text-rose-500 text-xs mt-1">
                    {generalForm.formState.errors.birthDate.message}
                  </p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <AlignLeft
                    className="absolute left-4 top-4 text-brand-text-muted group-focus-within:text-info transition-colors"
                    size={18}
                  />
                  <textarea
                    rows={4}
                ...
                {generalForm.formState.errors.bio && (
                  <p className="text-rose-500 text-xs mt-1">
                    {generalForm.formState.errors.bio.message}
                  </p>
                )}
              </div>
