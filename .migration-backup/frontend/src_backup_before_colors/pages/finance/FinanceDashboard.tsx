    ...
    try {
      ...
      if (isAdmin) {
        ...
        if (filters.status !== 'ALL')
          (params as unknown as Record<string, unknown>).status = filters.status;
        if (filters.type !== 'ALL')
          (params as unknown as Record<string, unknown>).type = filters.type;
        if (debouncedSearch) (params as unknown as Record<string, unknown>).search = debouncedSearch;
        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-5 xl:gap-6">
            <Card
              ...
              title={t('finance.revenueOverTime')}
            >
              <div className="h-[350px] mt-6">
                {monthlyData.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-8">
                    <p className="text-brand-text-sub font-bold">
                      {t('finance.noRevenueChart', 'No paid transactions yet to chart.')}
                    </p>
                  </div>
                ) : (
            ...
            {!isAdmin && studentStats && (
              <div className="flex items-center gap-6">
                ...
                <div className="text-end">
                  ...
                  <p className="text-xl font-black text-brand-primary-500">
                  ...
                  <TableRow key={payment.id}>
                    ...
                    {!isAdmin && (
                      <TableCell className="font-bold text-brand-text-main">
                        {t(`finance.${payment.type.toLowerCase()}`)}
                      </TableCell>
                    )}
