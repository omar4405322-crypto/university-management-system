...
const EditCollegeModal = ({ isOpen, onClose, college, onSuccess }) => {
  ...
  const getErrorMessage = (error) => {
    if (error.status === 403) {
      return t(
        'colleges.insufficientPermissions',
        'You do not have permission to edit colleges. Only Super Admins can edit colleges.'
      );
    }
    if (error.status === 401) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (result.success) {
        toast.success(t('colleges.updateSuccess', 'College updated successfully!'));
        onSuccess();
      }
    } catch (error: any) {
        ...
        <div className="space-y-5">
          <div className="space-y-1.5">
            <label className="text-sm font-bold text-brand-text-main flex items-center gap-2 ml-1">
              <School size={14} className="text-brand-text-muted" /> {t('colleges.nameEn')}{' '}
