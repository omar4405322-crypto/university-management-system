    ...
    >
      {({ isActive }) => (
        <>
          <item.icon
            size={isChild ? 16 : 20}
            className={`shrink-0 transition-all duration-300 ${isActive ? 'text-white scale-110' : 'text-brand-text-muted group-hover:text-brand-primary-500 group-hover:scale-110'}`}
          />
          {!isCollapsed && (
            <span
      ...
      >
        <div className="flex items-center gap-3">
          {GroupIcon && (
            <GroupIcon
              size={14}
              className="text-brand-text-muted group-hover:text-brand-primary-500 transition-colors"
            />
          )}
    ...
    <>
      ...
      >
        <div className="flex h-20 items-center justify-between border-b border-white/10 bg-black/10 px-4">
          <div
            className={`flex min-w-0 flex-1 items-center gap-3 ${isCollapsed ? 'hidden' : 'flex'}`}
          >
            <img
            ...
            <div className="flex min-w-0 flex-col">
              ...
              <span className="text-[10px] font-bold uppercase leading-tight tracking-tighter text-brand-primary-500">
            ...
            <div className={`flex items-center gap-4 ${isCollapsed ? 'justify-center' : 'px-2'}`}>
              ...
              {!isCollapsed && (
                <div className="flex-1 min-w-0">
                  ...
                  <p className="label-stat text-brand-primary-500 mt-1 opacity-80">
                    {user?.role.replace('_', ' ')}
                  </p>
                </div>
              )}
