        ...
        <div className="space-y-4">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-surface-subtle border-b border-brand-border">
                  <th className="px-4 py-3 text-start font-black uppercase tracking-widest text-[10px] text-brand-text-muted">{t('students.fullName')}</th>
                  <th className="px-4 py-3 text-start font-black uppercase tracking-widest text-[10px] text-brand-text-muted">{t('quizzes.score')}</th>
                  <th className="px-4 py-3 text-start font-black uppercase tracking-widest text-[10px] text-brand-text-muted">{t('quizzes.submittedAt')}</th>
                  <th className="px-4 py-3 text-end font-black uppercase tracking-widest text-[10px] text-brand-text-muted">{t('common.actions')}</th>
                </tr>
              </thead>
                  ...
                  <tr key={sub.id} className="hover:bg-surface-subtle/50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2 font-bold text-brand-text-main">
                        ...
                        <span className="text-[10px] text-brand-text-muted">({sub.student?.studentId})</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
          ...
          {selectedSubmission && (
            <div className="p-4 rounded-2xl bg-surface-subtle border border-brand-border">
              <h4 className="text-sm font-black text-brand-text-main mb-3">{t('quizzes.answersFor', { name: `${selectedSubmission.student?.firstName} ${selectedSubmission.student?.lastName}` })}</h4>
              {selectedSubmission.answers && typeof selectedSubmission.answers === 'object' ? (
                <ul className="space-y-2 text-sm">
                  {Object.entries(selectedSubmission.answers).map(([qId, answer]) => (
                    <li key={qId} className="flex justify-between gap-4 font-semibold text-brand-text-sub">
                      <span>{t('quizzes.question')} {qId}</span>
