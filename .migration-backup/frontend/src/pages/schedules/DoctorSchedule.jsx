...
const DoctorSchedule = () => {
  ...
  const totalClasses = Object.values(schedule).reduce(
    (sum, day) => sum + (Array.isArray(day) ? day.length : 0), 0
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                {t('common.dr', 'Dr.')} {user?.firstName} {user?.lastName} — {t('schedule.doctorScheduleSubtitle')}: {loading ? '...' : totalClasses}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-6 p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
            <Filter size={16} />
            <span className="text-xs font-bold uppercase tracking-widest">{t('common.filters', 'Filters')}</span>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap flex-1">
            ...
            {(selectedYear || selectedSemester) && (
              <button
                onClick={() => { setSelectedYear(''); setSelectedSemester(''); }}
            ...
            return (
              <div key={eng} className={`rounded-2xl border p-4 ${DAY_COLORS[eng]}`}>
                ...
                <div className="grid gap-2">
                  {entries.map((entry, i) => (
                    <div key={i} className="bg-white dark:bg-slate-800 rounded-xl p-3 flex flex-col md:flex-row md:items-center gap-2 shadow-sm">
