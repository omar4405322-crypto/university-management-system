...
export const createStudent = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (req.user && req.user.role === 'ADMIN') {
    if (!req.user.managedCollegeId) {
      return next(new AuthorizationError('Access denied: Unscoped admin cannot create students'));
    }
    if (studentData.departmentId) {
      const dept = await prisma.department.findUnique({
        where: { id: parseInt(studentData.departmentId as string, 10) },
      });
  ...
  } else if (req.user && req.user.role === 'COLLEGE_ADMIN' && req.user.managedCollegeId) {
    if (studentData.departmentId) {
      const dept = await prisma.department.findUnique({
        where: { id: parseInt(studentData.departmentId as string, 10) },
      });
