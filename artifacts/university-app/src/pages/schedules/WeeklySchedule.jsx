      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex flex-col gap-2">
          ...
          <h1 className="text-3xl font-black text-brand-text-main tracking-tight">
            {departmentName ? `${departmentName} - ` : ''}{t('timetables.title')}
          </h1>
          <div className="flex flex-wrap items-center gap-3">
            {academicYear && <Badge variant="info" className="font-black uppercase tracking-widest">{t('auth.year')} {academicYear}</Badge>}
            {semester && <Badge variant="neutral" className="font-black uppercase tracking-widest">{t('timetables.semester')} {semester}</Badge>}
            {collegeName && (
              <>
                              ...
                              <div className="space-y-2">
                                {entries.map((entry, idx) => (
                                  <div key={idx} className="rounded-2xl p-4 border border-brand-border shadow-sm transition-all hover:shadow-lg hover:scale-[1.02] bg-brand-bg-card dark:bg-brand-bg-elevated group/entry">
                                    ...
                                    <p className="text-sm font-black text-brand-text-main leading-tight group-hover/entry:text-brand-green transition-colors">{entry.course?.name}</p>
